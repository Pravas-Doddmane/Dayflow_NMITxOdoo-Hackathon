  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">
            Review employee time-off applications, inspect attached medical certificates, and approve/reject with remarks.
          </p>
        </div>
                ...
                {leaves.map((leave) => {
                  ...
                  return (
                    <tr key={leave.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition-colors">
                      ...
                      {/* Leave Type */}
                      <td className="py-3.5 px-4">
                        <span className="font-bold px-2.5 py-1 rounded-lg bg-indigo-50 dark:bg-indigo-950/70 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800/50 uppercase tracking-wider text-[10px]">
                          {leave.leaveType}
                        </span>
                      </td>
                      ...
                      <td className="py-3.5 px-4 max-w-xs">
                        <p className="text-slate-700 dark:text-slate-300 font-medium">
                          {leave.remarks ? `"${leave.remarks}"` : <span className="text-slate-400 italic">No specific reason entered</span>}
                        </p>
                        {leave.adminComment && (
          ...
          <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 text-xs space-y-2">
            ...
            <div>
              ...
              <span className="text-slate-800 dark:text-slate-200 font-medium">
                {reviewModal.leave?.remarks ? `"${reviewModal.leave.remarks}"` : 'No specific reason provided.'}
              </span>
            </div>

            {/* Medical Certificate in Modal */}
            {reviewModal.leave?.attachmentUrl && (
              <div className="mt-2 pt-2 border-t border-slate-200 dark:border-slate-700 flex items-center justify-between">
                ...
                <button
                  ...
                  onClick={() =>
                    setPreviewDoc({
                      url: reviewModal.leave.attachmentUrl,
                      employeeName: reviewModal.leave.employeeName,
                      employeeCode: reviewModal.leave.employeeCode,
                      leaveType: reviewModal.leave.leaveType,
                    })
                  }
                  className="px-2.5 py-1 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-[11px] rounded-lg shadow-sm flex items-center gap-1"
                >
            ...
            <textarea
              ...
              placeholder={
                reviewModal.action === 'APPROVE'
                  ? 'e.g. Approved. Please hand over urgent items before starting your leave.'
                  : 'e.g. Rejecting due to crucial project deliverables on these dates.'
              }
      ...
      <Modal
        ...
        subtitle={`Submitted by: ${previewDoc?.employeeName} (${previewDoc?.employeeCode})`}
      >
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between pt-2">
            <a
              href={previewDoc?.url}
              download={`Medical_Certificate_${previewDoc?.employeeCode || 'doc'}.png`}
